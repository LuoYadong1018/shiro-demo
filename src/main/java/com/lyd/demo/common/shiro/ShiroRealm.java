package com.lyd.demo.common.shiro;
...
public class ShiroRealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
        ...
        //查询角色和权限(这里根据业务自行查询)
        List<Role> sysRoleEntityList = sysRoleService.selectSysRoleByUserId(userId);
        for (Role sysRoleEntity:sysRoleEntityList) {
            rolesSet.add(sysRoleEntity.getRoleName());
            List<Menu> sysMenuEntityList = sysMenuService.selectSysMenuByRoleId(sysRoleEntity.getRoleId());
            for (Menu sysMenuEntity :sysMenuEntityList) {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
        ...
        User user = sysUserService.selectUserByName(username);
        //判断账号是否存在
        if (user == null) {
